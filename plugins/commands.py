...
async def start(client, message):
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        try:
            ...
            logger.error("ʜᴇʏ (message.from_user.mention) ᴘʟᴇᴀsᴇ ᴊᴏɪɴ ᴜᴘᴅᴀᴛᴇ ᴄʜᴀɴɴᴇʟ!")
            return
        btn = [
            [
        ...
        await client.send_message(
            ...
            text="**ᴘʟᴇᴀsᴇ ᴊᴏɪɴ ᴏᴜʀ ᴜᴘᴅᴀᴛᴇs ᴄʜᴀɴɴᴇʟ!**",
    ...
    button = InlineKeyboardButton('▶ ᴡᴀᴛᴄʜ ᴏɴʟɪɴᴇ', callback_data=f'generate_stream_link:{file_id}')
    # Create the inline keyboard markup with the button
    keyboard = InlineKeyboardMarkup([[button]])
    await client.send_cached_media(
        chat_id=message.from_user.id,
...
async def delete_all_index_confirm(bot, message):
    ...
    await message.answer('ʟᴏᴠᴇ ғʀᴏᴍ ᴍᴀᴠɪᴍᴏᴅs')
    await message.message.edit('sᴜᴄᴄᴇssғᴜʟʟʏ ᴅᴇʟᴇᴛᴇᴅ')
